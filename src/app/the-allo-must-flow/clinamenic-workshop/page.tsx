...
export default function Page() {
  ...
  return (
    <div className="min-h-screen bg-black flex flex-col relative">
      ...
      <div
        className={`fixed inset-0 z-50 flex items-start justify-center transition-transform duration-300 ease-in-out transform ${
          donationChartDrawerState === 'open' ? 'translate-y-0' : 'translate-y-[100vh]'
        }`}
      >
        {/* Overlay */}
        <div
          className={`absolute inset-0 bg-black opacity-50 transition-opacity duration-300 ease-in-out ${
            donationChartDrawerState === 'open' ? 'opacity-50' : 'opacity-0 pointer-events-none'
          }`}
          onClick={() => setDonationChartDrawerState('closed')}
        ></div>
        ...
        >
          {/* Close Button */}
          <button
            className="absolute top-2 right-2 text-black text-xl focus:outline-none focus:ring-2 focus:ring-black rounded"
            onClick={() => setDonationChartDrawerState('closed')}
          ...
          <div className="drawer-container w-full h-full relative">
            ...
            <div className="flex items-center justify-center h-full">
              <p className="text-black text-lg">Donation Chart Content Here</p>
            </div>
          </div>
        </div>
      </div>
